# 🚀 0x476974

👋 Welcome to **[0x476974](https://0x476974.github.io/ "0x476974")**!

The organization name `0x476974` is written in hexadecimal, which translates to `Git` in plain text. This is the official website of the **[0x476974](https://github.com/0x476974 "0x476974")** GitHub organization 🏨. All projects and content on this website and in the organization are created and maintained by 💖 **[pxoqxo](https://github.com/pxoqxo "pxoqxo")**.

This is my personal playground 🥎 for learning, testing, experimenting, and documenting interesting technological knowledge 📚.

Everything in this organization is free and open source 🧰. You are free to use anything you find here, but use it at your own risk. There are no guarantees or warranties provided 🔴.

Check out my work below, and if you have any questions, feel free to reach out to me at [pxoqxo@atomicmail.io](mailto:pxoqxo@atomicmail.io "pxoqxo@atomicmail.io").

## My Works

- [⚡ Setup C++ Third-Party Library](https://github.com/0x476974/setup-cpp-third-party-library "⚡ Setup C++ Third-Party Library")
- [📐 Standard sizes for logo](https://github.com/0x476974/standard-sizes-for-logo "📐 Standard sizes for logo")
- [⚡ Quick GitHub Pages Tutorial by pxoqxo](https://github.com/0x476974/github-pages "⚡ Quick GitHub Pages Tutorial by pxoqxo")
- [📚 GitHub Special Repositories](https://github.com/0x476974/github-repository "📚 GitHub Special Repositories")
- [🐙 GitHub Accounts](https://github.com/0x476974/github-accounts "🐙 GitHub Accounts")
- [🚀 0x476974](https://github.com/0x476974/0x476974.github.io "🚀 0x476974")
- [🚀 0x476974](https://github.com/0x476974/0x476974 "🚀 0x476974")
- [🚀 .github](https://github.com/0x476974/.github "🚀 .github")

Thanks for visiting! ✨🚀